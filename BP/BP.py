...
def load_dataset(dataset_path, n_train_data):
    ...
    """
    dataset = []
    #label_dict = {'Dairy': 0, 'Fats': 1, 'Meat': 2,'Vegetables':3,'Fruits':4,'Breads':5,'Desserts':6}
    label_dict = {'aa': 0, 'bb': 1, 'cc': 2}
    with open(dataset_path, 'r') as file:
        ...
        for row in csv_reader:
            # 将字符串类型的特征值转换为浮点型
            row[0:m] = list(map(float, row[0:m]))
            # 将标签替换为整型
            row[m] = label_dict[row[m]]
            # 将处理好的数据加入数据集中
            dataset.append(row)
    ...
    for i in range(dataset.shape[1] - 1):
        dataset[:, i] = mms.fit_transform(
            dataset[:, i].reshape(-1, 1)).flatten()
...
# 结果可视化
# 特征有7个维度，类别有1个维度，一共8个维度，故采用了RadViz图
def result_visualization(x_test, y_test, predicted_label):
...
if __name__ == "__main__":
    file_path = 'D:\\python\\lyh\\MachineLearning\\BP\\hh.csv'
    start_time = datetime.datetime.now()
    # 参数设置
    l_rate = 0.2  # 学习率
    epochs = 500  # 迭代训练的次数
    n_hidden = 5  # 隐藏层神经元个数
    n_train_data = 280 # 训练集的大小（总共300条数据，训练集280条，验证集20条）

    # 加载数据并划分训练集和验证集
    train_data, val_data = load_dataset(file_path, n_train_data)
    # 训练模型
    network = train(train_data, l_rate, epochs, n_hidden, val_data)
    accuracy = validation(network, val_data)
    #获取预测标签
    predicted_label = fun(network, val_data)
    #获取真实标签
    actual_label = [row[-1] for row in val_data]
    val_data = np.array(val_data)
    val_data = val_data[:, 0:7].T
    ...
    #结果可视化
    result_visualization(val_data, actual_label, predicted_label)
    print(accuracy)
