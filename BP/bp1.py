...
'''def result_visualization(x_test, y_test, result):
    ...
    print(prediction)
    # 转换成DataFrame类型，并添加columns
    #df_real = pd.DataFrame(real, index=None, columns=['reason1', 'reason2', 'reason3', 'reason4', 'reason5'])
    #df_prediction = pd.DataFrame(prediction, index=None, columns=['reason1', 'reason2', 'reason3', 'reason4', 'reason5'])

    # 将特征列转换为float类型，否则radviz会报错
    #df_real[['reason1', 'reason2', 'reason3', 'reason4', 'reason5']] = df_real[['reason1', 'reason2', 'reason3', 'reason4', 'reason5']].astype(float)
    #df_prediction[['reason1', 'reason2', 'reason3', 'reason4', 'reason5']] = df_prediction[['reason1', 'reason2', 'reason3', 'reason4', 'reason5']].astype(float)

    # 转换成DataFrame类型，并添加columns
    df_real = pd.DataFrame(real, index=None,columns=['1', '2', '3', '4', '5', '6', '7', 'Species'])
    df_prediction = pd.DataFrame(prediction, index=None,columns=['1', '2', '3', '4','5', '6', '7', 'Species'])

    # 将特征列转换为float类型，否则radviz会报错
    df_real[['1', '2', '3', '4','5', '6', '7']] = df_real[
        ['1', '2', '3', '4','5', '6', '7']].astype(float)
    df_prediction[['1', '2', '3', '4','5', '6', '7']] = df_prediction[
        ['1', '2', '3', '4','5', '6', '7']].astype(float)
    # 绘图
    plt.figure('真实分类')
    radviz(df_real, 'Species', color=['blue', 'green', 'red', 'yellow', 'white', 'black', 'grey'])
    plt.figure('预测分类')
    radviz(df_prediction, 'Species', color=['blue', 'green', 'red', 'yellow', 'white', 'black', 'grey'])
    plt.show()
'''

if __name__ == "__main__":
    # 读取数据
    data_set = pd.read_csv('D:\\python\\lyh\\MachineLearning\\BP\\iris_training.csv', header=None)
    ...
    # 对模型进行测试
    data_test = pd.read_csv('D:\\python\\lyh\\MachineLearning\\BP\\iris_test.csv', header=None)
