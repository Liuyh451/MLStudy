...
class SVM:
    ...
    def __init__(self, trainDataList, trainLabelList, sigma = 10, C = 200, toler = 0.001):
        ...
        '''
        self.trainDataMat = np.mat(trainDataList)       #训练数据集np.mat()函数用于将输入解释为矩阵。
    ...
    def calcKernel(self):
        ...
        :return: 高斯核矩阵
        '''
        #初始化高斯核结果矩阵 大小 = 训练集长度m * 训练集长度m,多少个样本
        #k[i][j] = Xi * Xj
        ...
        for i in range(self.m):
            ...
            #不能每次都打印，会极大拖慢程序运行速度
            #因为print是比较慢的
            if i % 100 == 0:#这个100应该要改，暂定20
                print('construct the kernel:', i, self.m)
            #得到式7.90中的X
            X = self.trainDataMat[i, :]#训练集的第i行所有列
            #小循环遍历Xj，Xj为式7.90中的Z
...
if __name__ == '__main__':
    ...
    trainDataList, trainLabelList = loadData('data/data73555/mnist_train.csv')

    # 获取测试集及标签
    print('start read testSet')
    testDataList, testLabelList = loadData('data/data73555/mnist_test.csv')
