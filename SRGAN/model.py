...
def SRGAN_g(t_image, is_train=False, reuse=False):
    ...
    """
    # 权重初始化器
    w_init = tf.random_normal_initializer(stddev=0.02)  # 这个初始化器生成服从均值为 0、标准差为 0.02 的正态分布的权重值,通常用于初始化卷积层的权重
    # 偏置初始化器
    b_init = None  # tf.constant_initializer(value=0.0)
    # gamma缩放因子初始化器,生成服从均值为 1、标准差为 0.02 的正态分布的值
    g_init = tf.random_normal_initializer(1., 0.02)
    # 定义一个变量作用域，作用域名称为 "SRGAN_g"。如果 reuse=True，表示重用变量。
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        # tl.layers.set_name_reuse(reuse) # remove for TL 1.8.0+
        # 创建一个输入层 `InputLayer`，`t_image` 是输入张量，层的名称为 'in'。
...
def SRGAN_g2(t_image, is_train=False, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        n = UpSampling2dLayer(n, size=[size[1] * 2, size[2] * 2], is_scale=False, method=1, align_corners=False,
                              name='up1/upsample2d')
        n = Conv2d(n, 64, (3, 3), (1, 1), padding='SAME', W_init=w_init, b_init=b_init,
                   name='up1/conv2d')  # <-- may need to increase n_filter
        n = BatchNormLayer(n, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='up1/batch_norm')

        n = UpSampling2dLayer(n, size=[size[1] * 4, size[2] * 4], is_scale=False, method=1, align_corners=False,
                              name='up2/upsample2d')
        n = Conv2d(n, 32, (3, 3), (1, 1), padding='SAME', W_init=w_init, b_init=b_init,
                   name='up2/conv2d')  # <-- may need to increase n_filter
...
def SRGAN_d2(t_image, is_train=False, reuse=False):
    ...
    feature maps (n) and stride (s) feature maps (n) and stride (s)
    定义了 SRGAN 生成器的一种变体，名为 SRGAN_g2，用于将低分辨率图像（例如 96x96）放大到高分辨率图像（例如 384x384）。
    该实现使用了上采样层（UpSampling2dLayer）代替了传统的反卷积层
    """
    w_init = tf.random_normal_initializer(stddev=0.02)
    b_init = None
    g_init = tf.random_normal_initializer(1., 0.02)
...
#判别器，改了论文的结构，改为了k4n64*hs2了，有助于提取特征简化计算等
def SRGAN_d(input_images, is_train=True, reuse=False):
    ...
    df_dim = 64
    lrelu = lambda x: tl.act.lrelu(x, 0.2)
    #tf.variable_scope 是 TensorFlow 中用于管理变量和命名空间的功能。它允许你创建一个变量的命名空间，使得在这个命名空间下创建的所有变量都带有特定的前缀
